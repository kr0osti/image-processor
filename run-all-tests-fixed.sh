#!/bin/bash
# Script to run all tests with fixed configurations
...
if [ -z "$API_KEY" ]; then
  ...
  # Escape any special characters in the API key for JavaScript string
...
# Create a fixed version of the cleanup test
...
describe('Cleanup API', () => {
  ...
  it('should run cleanup with default maxAge if a valid API key is provided', async () => {
    ...
    const req = new NextRequest(\`http://localhost:3000/api/cleanup?apiKey=\${process.env.CLEANUP_API_KEY}\`);
  ...
  it('should run cleanup with custom maxAge if provided', async () => {
    ...
    const req = new NextRequest(\`http://localhost:3000/api/cleanup?apiKey=\${process.env.CLEANUP_API_KEY}&maxAge=7200000\`);
